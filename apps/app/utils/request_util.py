...
from apps.app.utils.authentication import RedisOpAuthJwtAuthentication
...
def get_request_user(request):
    ...
    if user and user.is_authenticated:
        return user
    try:
        user, token = RedisOpAuthJwtAuthentication().authenticate(request)
...
def get_login_location(request, *args, **kwargs):
    ...
    # 从缓存中获取
    location = cache.get(request_ip) if getattr(settings, "REDIS_ENABLE", True) else ""
    if location:
        return location
...
def save_login_log(request, login_type=1):
    ...
    LoginInfor.objects.create(**analysis_data)
