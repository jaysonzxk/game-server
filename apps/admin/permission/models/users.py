...
class UserProfile(AbstractUser, CoreModel):
    ...
    inviteCode = CharField(max_length=20, verbose_name="邀请码", null=True)
    parent = ForeignKey(to='permission.UserProfile', verbose_name='上级用户', on_delete=CASCADE, db_constraint=False,
                        null=True, blank=True, related_name='parentId')
    balance = DecimalField(max_digits=15, decimal_places=2, verbose_name='余额', null=True, blank=True)
    gender = IntegerField(verbose_name="性别", default=1, null=True, blank=True)
    remark = TextField(verbose_name="备注", null=True)
    status = IntegerField(verbose_name='状态', null=True, blank=True, default=0)
