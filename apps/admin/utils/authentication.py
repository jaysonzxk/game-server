...
from apps.admin.permission.models.users import UserProfile
from apps.admin.utils.jwt_util import jwt_get_session_id
...
class RedisOpAuthJwtAuthentication(OpAuthJwtAuthentication):
    ...
    def authenticate(self, request):
        ...
        if res and getattr(settings, "REDIS_ENABLE", False):
            ...
            if redis_token and (json.loads(redis_token).get('token')  == token):
                return user, token
            else:
